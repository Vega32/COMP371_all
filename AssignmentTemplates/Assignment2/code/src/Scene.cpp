...
void Scene::processFile(string s)
{
	...
	generateScreenSpaceCoordinates();
...
glm::vec3 Scene::getScreenVertexByIndex(int index)
{
	return glm::vec3(
		screenSpaceVector[index * 3],
		screenSpaceVector[index * 3 + 1],
		screenSpaceVector[index * 3 + 2]
	);
}

void Scene::generateScreenSpaceCoordinates() {
	screenSpaceVector = vertecies;
	glm::mat4 mv;
	for (int i = 0; i < 16; i++) {
		mv[i / 4][i % 4] = modelViewMatrix[i];
	}
	glm::mat4 proj;
	for (int i = 0; i < 16; i++) {
		proj[i / 4][i % 4] = projectionMatrix[i];
	}
	for (int i = 0; i < numVertiecies; i++) {

		glm::vec3 v = getVertexByIndex(i);

		glm::vec4 tempV = (proj * mv * glm::vec4(v, 1.0));
		glm::vec3 vertex = glm::vec3(tempV) / tempV.w;
		vertex.x = (vertex.x + 1.0f) * 0.5 * dimensions[0];
		vertex.y = dimensions[1] - (vertex.y + 1.0f) * 0.5 * dimensions[1];

		screenSpaceVector.at(i * 3) = vertex.x;
		screenSpaceVector.at(i * 3 + 1) = vertex.y;
		screenSpaceVector.at(i * 3 + 2) = vertex.z;
	}
}
...
void Scene::getNormals()
{
	...
	for (int i = 0; i < numTriangles; i++) {
		...
		beryData.push_back(vertexNormals[i1].x);
		beryData.push_back(vertexNormals[i1].y);
		beryData.push_back(vertexNormals[i1].z);
		beryData.push_back(v1.x);
		beryData.push_back(v1.y);
		beryData.push_back(v1.z);
		...
		beryData.push_back(v2.y);
		beryData.push_back(v2.z);
		beryData.push_back(vertexNormals[i2].x);
		beryData.push_back(vertexNormals[i2].y);
		beryData.push_back(vertexNormals[i2].z);
		beryData.push_back(v1.x);
		beryData.push_back(v1.y);
		beryData.push_back(v1.z);
		beryData.push_back(v2.x);
		...
		beryData.push_back(v3.y);
		beryData.push_back(v3.z);
		beryData.push_back(vertexNormals[i3].x);
		beryData.push_back(vertexNormals[i3].y);
		beryData.push_back(vertexNormals[i3].z);
		beryData.push_back(v1.x);
		beryData.push_back(v1.y);
		beryData.push_back(v1.z);
		beryData.push_back(v2.x);
...
void Scene::createBuffers()
{
	...
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, beryIndices.size() * sizeof(unsigned int), beryIndices.data(), GL_STATIC_DRAW);
